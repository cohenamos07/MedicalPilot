 ...
 * Updated: 19/04/2026
 * Service: S07
 * תיקון: שמות מודלים נכונים — gemini-2.0-flash-lite + gemini-1.5-flash-latest
 */
...
function _getDocTextViaDriveExport(docId) {
  ...
  if (code !== 200) throw new Error("שגיאת Drive Export: " + code);
...
function _callGeminiWithModel(model, prompt, apiKey) {
  ...
  Logger.log("תשובה (200 תווים): " + rawText.substring(0, 200));
...
function _callGemini_S07(text, examples) {
  ...
  const prompt =
    ...
    "{\"title\":\"סוג המסמך\",\"issuer\":\"שם המנפיק\",\"classification\":\"מסמך רפואי או מסמך חשבונאי או ביטוח או אחר\",\"complexity\":\"פשוט או מורכב\"}\n\n" +
  ...
  Logger.log("מנסה gemini-2.0-flash-lite...");
  const r20 = _callGeminiWithModel("gemini-2.0-flash-lite", prompt, apiKey);
  if (r20.ok) { Logger.log("✅ הצליח 2.0-flash-lite"); return r20.data; }

  Logger.log("עובר ל-gemini-1.5-flash-latest...");
  const r15 = _callGeminiWithModel("gemini-1.5-flash-latest", prompt, apiKey);
  if (r15.ok) { Logger.log("✅ הצליח 1.5-flash-latest"); return r15.data; }

  if (r20.quotaExceeded && r15.quotaExceeded) throw new Error("מכסה מוצתה בשני המודלים");
  throw new Error("שגיאת AI: " + (r15.errorMsg || r20.errorMsg || "לא ידוע"));
}